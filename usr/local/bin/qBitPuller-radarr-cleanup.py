...
def log(msg: str) -> None:
    print(f"[qBitPuller-radarr] {msg}", flush=True)
...
def get_config() -> Config:
    ...
    try:
        radarr_timeout = int(env.get("RADARR_TIMEOUT", "30"))
    except ValueError:
        raise SystemExit("Config invalide: RADARR_TIMEOUT doit etre un entier (secondes)")

    try:
        min_age_minutes = int(env.get("RADARR_MIN_AGE_MINUTES", "60"))
    except ValueError:
        raise SystemExit("Config invalide: RADARR_MIN_AGE_MINUTES doit etre un entier (minutes)")
    try:
        history_since_days = int(env.get("RADARR_HISTORY_SINCE_DAYS", "14"))
    except ValueError:
        raise SystemExit("Config invalide: RADARR_HISTORY_SINCE_DAYS doit etre un entier (jours)")

    dest_root = os.path.realpath(req("DEST_ROOT"))
    radarr_subdir = (env.get("RADARR_SUBDIR") or "radarr").strip().strip("/")
    dry_run = bool_from_env(env.get("RADARR_CLEANUP_DRY_RUN"), True)
    clean_empty_dirs = bool_from_env(env.get("RADARR_CLEANUP_EMPTY_DIRS"), True)

    return Config(
        radarr_url=req("RADARR_URL").rstrip("/") + "/",
        radarr_api_key=req("RADARR_API_KEY"),
        radarr_timeout=radarr_timeout,
        dest_root=dest_root,
        radarr_subdir=radarr_subdir,
        dry_run=dry_run,
...
def build_imported_paths(client: RadarrClient, since_days: int) -> List[str]:
    paths: Set[str] = set()
    ...
    log(f"Requete Radarr: /api/v3/history/since?date={date_iso}&eventType=downloadFolderImported")
...
def main() -> int:
    cfg = get_config()
    lock_path = "/var/lock/qBitPuller-radarr-cleanup.lock"
    ...
    try:
        ...
        target_root = os.path.realpath(os.path.join(cfg.dest_root, cfg.radarr_subdir))

        if not os.path.isdir(target_root):
            raise SystemExit(f"DEST_ROOT ou RADARR_SUBDIR invalide: dossier introuvable {target_root}")

        log("Lecture Radarr...")
        client = RadarrClient(cfg.radarr_url, cfg.radarr_api_key, timeout=cfg.radarr_timeout)
